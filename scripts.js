(function() {
	...
	$('.colum').on('click', function(e) {
		...
		for (var i = 5; i >= 0; i--) {
			if (!slotInCol.eq(i).hasClass('player1') && !slotInCol.eq(i).hasClass('player2')) {
	...
	function switchPlayer() {
		...
		if (currentPlayer === 'player1') {
			...
			turn = '<p style="background-color: #533B4D;border-radius: 25px; color:#fae3c6; animation: shake .4s ease-out 3s ">Player 2</p>'; //creates text
		} else {
			currentPlayer = 'player1';
			turn = '<p style="background-color: #bb206b;border-radius: 25px; color:#fae3c6; animation: shake .4s ease-out 3s ">Player 1</p>';
		}
